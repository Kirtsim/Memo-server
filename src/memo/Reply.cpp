...
{
    std::vector<boost::asio::const_buffer> aBuffers;
    ...
    aBuffers.push_back(boost::asio::buffer(kCRLF));
    aBuffers.push_back(boost::asio::buffer(content));
    return aBuffers;
}
...
std::string ToString(Reply::Status iStatus)
{
    switch (iStatus)
    {
    case Reply::ok:
...
Reply Reply::StockReply(Reply::Status iStatus)
{
    Reply aReply;
